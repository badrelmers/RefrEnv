...
getNewlyAddedVars(){ 
    ...
    # TODO: add zsh critical variables, for now I added only variables starting with zsh... and PROMPT PROMPT2 PROMPT3 PROMPT4 (PROMPT is very important to remove it otherwise the zsh prompt get changed, cmd and zsh use PROMPT so we should never overwrite PROMPT)
    local regresult
